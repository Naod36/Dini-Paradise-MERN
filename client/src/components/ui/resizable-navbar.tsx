  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isvisible
          ? "rounded-3xl bg-white/80 dark:bg-neutral-900/80"
          : " bg-black/40",
        className
      )}
    >
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-3xl bg-gray-100/30 dark:bg-neutral-800/30 z-10"
            />
          )}
          <span
            className={cn(
              "relative z-20 transition-colors duration-200",
              isvisible ? "text-black/80 dark:text-white" : "text-white/90"
            )}
          >
            {item.name}
          </span>
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isvisible ? "bg-white/80  dark:bg-neutral-950/80" : " bg-black/60",
        className
      )}
    >
      {React.Children.map(children, (child) =>
      ...
      {isOpen && (
        <motion.div
          ...
          className={cn(
            ...
            isvisible
              ? "bg-white/80 text-black/90 dark:bg-neutral-950/80"
...
export const NavbarLogo = ({ isvisible }: NavbarLogoProps) => {
  return (
    <a
      href="/"
      className="relative z-10 mr-4 flex items-center space-x-2 px-2 py-1 text-sm font-normal text-black"
    >
      <img src={smallLogo} alt="Dini Paradise Logo" width={40} height={40} />
      <span
        className={cn(
          "font-medium font-marhey text-lg text-black dark:text-white",
          isvisible ? " text-black/80 dark:text-white" : "text-white"
        )}
      >
        Dini Paradise
      </span>
